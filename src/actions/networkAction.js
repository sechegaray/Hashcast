...
export function transfer (data) {
  return async function (dispatch) {
    dispatch({ type: `TRANSFER/CREATE/REQUEST` });
    try {
      const response = await networkService.transfer(data);
      dispatch({ type: `TRANSFER/CREATE/SUCCESS`, payload: response });
      return response;
    } catch (error) {
      dispatch({ type: 'TRANSFER/CREATE/ERROR' });
